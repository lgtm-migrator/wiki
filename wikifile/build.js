...
const writeNavigationFile = () => {
  ...
  writeNavigationLine()
  getFilesFromDir(tree).forEach(item => item.name == 'navigation.md' ? writeNavigationLine() : writeNavigationLine(makeLink(item)) & writeNavigationLine())
  getDirsFromDir(tree).forEach((dir) => {
    ...
    getDirsFromDir(dir).forEach((dir, idx, arr) => {
      ...
      if (arr.length - 1 != idx) writeNavigationLine('---')
    })
    writeNavigationLine()
  })
